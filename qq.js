...
module.exports = {
  ...
  async search(query, page = 1) {
    ...
    try {
      const res = await fetch(
        `https://api2.wer.plus/api/qqsearch?keywords=${kw}&limit=30`
      );
      const j = await res.json();
      if (j && j.data && j.data.song) {
  ...
  async getMediaSource(item) {
    try {
      ...
      const res = await fetch(
        `https://api2.wer.plus/api/qqplay?mid=${mid}`
      );
      const j = await res.json();
  ...
  async getLyric(item) {
    try {
      const mid = item.id.replace("qq_", "");
      const res = await fetch(
        `https://api2.wer.plus/api/qqlyric?mid=${mid}`
      );
      const j = await res.json();
